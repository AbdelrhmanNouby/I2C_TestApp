...
uint8_t TIMER0_u8Init (void)
{
	...
	// error checking 
	if ( GET_BIT(TCCR0_REG, WGM00) == 0 && GET_BIT(TCCR0_REG, WGM01) == 1 && GET_BIT(SREG_REG, I_BIT) == 1 && GET_BIT(TIMSK_REG, OCIE0) == 1	)	
...
uint8_t TIMER0_u8Start (void)
{
	...
	// error checking 
	if ( GET_BIT(TCCR0_REG, CS00) == 1 && GET_BIT(TCCR0_REG, CS01) == 1 && GET_BIT(TCCR0_REG,CS02) == 0)
		u8ErrorState = STD_TYPES_OK ;
	else
...
uint8_t TIMER0_u8Stop (void)
{
	...
	// error checking 
	if ( GET_BIT(TCCR0_REG, CS00) == 0 && GET_BIT(TCCR0_REG, CS01) == 0 && GET_BIT(TCCR0_REG,CS02) == 0 && GET_BIT(TIMSK_REG,OCIE0) == 0 )
		u8ErrorState = STD_TYPES_OK ;
	else
		u8ErrorState = Timer0_Start_Error ;
